...
function loadNotifications(uid) {
  ...
  const q = query(
    ...
    limit(80), // evitamos índice compuesto; ordenamos en cliente
  );
  ...
  const bind = () =>
    onSnapshot(
      ...
      async (err) => {
        console.error("Firestore error (notificaciones):", err);
        try {
          const warm = await getDocs(baseQuery);
          applyRows(warm.docs);
          showToast("Notificaciones listas", "He activado modo compatible sin índice.", "info");
        } catch {}
        notifListEl.innerHTML = `
          <div class="empty-state-v8">
            <i class="fas fa-exclamation-triangle opacity-20 text-red-400 mb-4 text-4xl"></i>
