...
import { initAppUI } from "./ui-core.js?v=8.0";
import { injectHeader, injectNavbar } from "./modules/ui-loader.js?v=8.0";
...
function getPlayerDisplayName(uid) {
  ...
  if (uid === currentUser?.uid)
    return currentUserData?.nombreUsuario || currentUserData?.nombre || "TÃƒÅ¡";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  observeCoreSession({
    ...
    onReady: async ({ user, userDoc }) => {
      ...
      queueNonCriticalHomeBoot();
      renderWelcome();
      bindTabs();
      queueDeferredHomeTasks(userDoc);
      ...
      try {
        ...
        scheduleDeferredCollectionBoot();
      } catch (err) {
...
async function checkHomeNotices() {
    ...
    if (todayMatches.length > 0) {
        notices.push({
            type: 'game',
            title: 'Ã‚Â¡HOY JUEGAS!',
            message: `Tienes ${todayMatches.length} ${todayMatches.length > 1 ? 'partidos' : 'partido'} programado para hoy. Ã‚Â¡A por todas!`,
    ...
    if (missingDiary.length > 0) {
        notices.push({
            ...
            message: "No has apuntado tus datos en el diario del ÃƒÂºltimo partido. Ã‚Â¡No pierdas tu racha tÃƒÂ¡ctica!",
...
function queueDeferredHomeTasks(userDoc) {
  runWhenIdle(() => initNexus(), 1400);
  setTimeout(() => bindNotificationNudge(), 450);
  setTimeout(() => checkSystemAlerts(userDoc), 900);
  setTimeout(() => checkHomeNotices(), 1200);
}

function scheduleDeferredCollectionBoot() {
  runWhenIdle(async () => {
    try {
      unsubEv = await subscribeCol(
        "eventoPartidos",
      ...
      console.warn("Deferred event matches load failed:", err);
    }
  }, 1600);
  ...
  runWhenIdle(async () => {
    try {
      ...
      console.warn("Deferred events load failed:", err);
    }
  }, 1900);
}
function queueNonCriticalHomeBoot() {
  const run = () => {
    injectHeader(currentUserData).catch(() => {});
    injectNavbar("home").catch(() => {});
    initWeather().catch(() => {});
  };
...
async function initWeather() {
  try {
    ...
    if (tempEl && weather?.current) {
      tempEl.textContent = `${Math.round(weather.current.temperature_2m || 0)}Ã‚Â°`;
    }
    if (weather?.current) {
...
async function checkSystemAlerts(userData) {
  ...
  if (today.length > 0) {
    ...
    alerts.push({
      title: "Partido Hoy",
      body: `Ã‚Â¡Hoy tienes partido a las ${time}! PrepÃƒÂ¡rate para la victoria.`,
  ...
  if (lastPlayed) {
    ...
    if (!hasDiary) {
      alerts.push({
        title: "Diario Pendiente",
        body: "No has registrado tus sensaciones del ÃƒÂºltimo partido. Ã‚Â¡Suma puntos extra ahora!",
        icon: "fa-book",
...
function refreshRecommendations() {
  ...
  if (typeof weather !== "undefined" && weather?.current) {
    ...
    if (code <= 1)
      recs.push(
        "Ã¢Ëœâ‚¬Ã¯Â¸Â Hoy el tiempo es soleado, ideal para un partido exterior. Ã‚Â¡Aprovecha!",
      );
    else if (code > 50)
      recs.push(
        "Ã¢Ëœâ€ Parece que va a llover. Intenta buscar partidos en pistas cubiertas (Indoor).",
      );

    if (wind > 15) {
      recs.push(
        `Ã°Å¸Å’Â¬Ã¯Â¸Â Viento fuerte hoy (${wind} km/h). Ten cuidado con los globos largos y ajusta tu posicionamiento al viento.`,
      );
    }
  }
  ...
  if (typeof allMatches !== "undefined" && allMatches.length > 0) {
    ...
    if (todayMatches.length > 0) {
      recs.push(
        `Ã°Å¸Å½Â¾ Hoy hay mucho movimiento en la red. Hay ${todayMatches.length} partidos programados hoy. Ã‚Â¿Ya tienes el tuyo?`,
      );
    }
  }
  ...
  // Tips tÃƒÂ¡cticos
  recs.push(
    "Ã°Å¸â€Â¥ Mejora tu derecha: mantÃƒÂ©n la pala alta en la red y flexiona bien las piernas al defender el fondo de pista.",
  );
  recs.push(
    'Ã°Å¸Â§Â  Recuerda comunicarte constantemente con tu compaÃƒÂ±ero: "MÃƒÂ­a", "Tuya", "Corto", "Largo". La comunicaciÃƒÂ³n gana partidos.',
  );
  recs.push(
    'Ã°Å¸â€™Âª DespuÃƒÂ©s de un buen partido, pasa por "DIARIO" y apunta tus sensaciones para sumar puntos VIP a tu ranking.',
  );
  recs.push(
    "Ã°Å¸Ââ€  Analiza a tus posibles rivales antes de entrar a pista y crea una tÃƒÂ¡ctica. Piensa dÃƒÂ³nde estÃƒÂ¡n sus puntos dÃƒÂ©biles.",
  );

  const idx = Math.floor(Math.random() * recs.length);
  recomEl.innerHTML = `<span class="animate-fade-in inline-block">${recs[idx]}</span>`;

  // Cambiar recomendaciÃƒÂ³n cada pocos segundos de forma cÃƒÂ­clica
  ...
  if (nextEvent) {
    ...
    cards.push(`
      <div class="hv2-event-chip">
        ...
        <div class="hv2-event-chip-sub">Estado: ${String(nextEvent.estado || "inscripcion").toUpperCase()} Ã‚Â· ${inscritos}/${Number(nextEvent.plazasMax || 16)} inscritos</div>
      </div>
    `);
  }
    ...
    upcoming.forEach((m) => {
      ...
      cards.push(`
        <div class="hv2-event-chip" onclick="window.openMatch('${m.id}','${m.col}')">
          <div class="hv2-event-chip-title">PARTIDO EVENTO Ã‚Â· ${String(m.phase || "evento").toUpperCase()}</div>
          <div class="hv2-event-chip-sub">${m.teamAName || "TBD"} vs ${m.teamBName || "TBD"} Ã‚Â· ${when}</div>
        </div>
      `);
    });
  } else {
...
function renderNextMatch() {
  ...
  const next = mine[0];
  if (!next) {
    box.innerHTML = `<div class="hv2-no-match"><i class="fas fa-calendar-xmark"></i><span>Sin prÃƒÂ³ximo partido programado</span></div>`;
    return;
  }
  ...
  const tempVal = weather?.current
    ...
    : null;
  const temp = tempVal !== null ? `${tempVal}Ã‚Â°` : "--";
  const tempColor =
    tempVal !== null
      ? tempVal >= 30
        ? "#ef4444"
...
function renderMatchCard(match, idx = 0) {
  ...
  const badge = finished
    ...
    : isEvent
      ? `<span class="hv2-mc-badge open-badge">EVENTO${match.phase ? ` Ã‚Â· ${String(match.phase).toUpperCase()}` : ""}</span>`
      : isReto
...
window.openNexusModal = () => {
  ...
  if (title) title.textContent = `Usuarios Ã‚Â· ${nexusOnlineUsers.length} conectados / ${offlineUsers.length + nexusOnlineUsers.length} total`;
      ...
      return `
        <div class="nexus-modal-row" onclick="window.location.href='perfil.html?uid=${u.id}'">
          ...
          <div class="nexus-modal-info">
            <span class="nexus-modal-name">${isMe ? "TÃƒÅ¡" : name}</span>
  ...
  const offlineHtml = `
    <div class="nexus-modal-divider">No conectados Ã¢â‚¬â€ ÃƒÂºltima actividad (${offlineUsers.length})</div>
    ${
      offlineUsers.length
        ? offlineUsers
            .map((u) => {
              ...
              return `
                <div class="nexus-modal-row is-offline" onclick="window.location.href='perfil.html?uid=${u.id}'">
                  ...
                  <div class="nexus-modal-info">
                    ...
                    <span class="nexus-modal-meta">ÃƒÅ¡ltimo acceso: ${lastTxt}</span>
                  </div>
                </div>
              `;
            })
            .join("")
        : '<div class="text-[10px] opacity-40 p-2">Todos estÃƒÂ¡n conectados o sin datos.</div>'
    }
  `;
...
window.clearAppCache = async () => {
  try {
    ...
    window.__appToast?.("CachÃƒÂ© limpiada", "Recargando interfaz...", "success");
    setTimeout(() => window.location.reload(), 450);
  } catch (e) {
    console.warn("clear cache failed:", e);
    window.__appToast?.("Error", "No se pudo limpiar la cachÃƒÂ©", "error");
  }
};
