...
export async function injectNavbar(activePage) {
    ...
    scheduleVecinaChatInit();
}
let vecinaChatScheduled = false;
function scheduleVecinaChatInit() {
    if (vecinaChatScheduled || isPublicPage()) return;
    vecinaChatScheduled = true;
    const boot = async () => {
        try {
    ...
    if (typeof window !== 'undefined' && typeof window.requestIdleCallback === 'function') {
        window.requestIdleCallback(() => boot(), { timeout: 1800 });
        return;
    }
    setTimeout(() => {
        boot();
