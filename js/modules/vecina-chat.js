...
function _findUserByName(name) {
  ...
  const exact = DATA_CACHE.globalUsers.find(u => (u.nombreUsuario || u.nombre || "").toLowerCase() === q);
...
function _generateResponse(intent, query) {
  ...
  if (intent === "CMD_VICTIM") {
    ...
    if (!victim) return currentPersonality === 'vecina' 
        ? "Mira, pichón... todavía no has 'ajusticiado' a nadie en la pista. ¡Hay que ser más agresivo en la red!" 
        : "No se ha detectado un rival con tendencia de derrota significativa en tu contra.";
    
    const vName = victim.nombreUsuario || victim.nombre;
    const vStats = victim.stats;
    const total = vStats.wins + vStats.losses;
    const winRate = Math.round((vStats.wins / total) * 100);
    
    if (currentPersonality === 'vecina') {
       return `¡Ay! Tu 'hijo' oficial es **${vName}**. Le has ganado ${vStats.wins} de ${total} partidos (un ${winRate}%). ¡Pobrecillo, si te ve ya se echa a temblar y busca la salida!`;
    }
    return `Análisis de Dominio Táctico: Tu rival más vulnerable es **${vName}**. Historial H2H: ${vStats.wins}V - ${vStats.losses}D (${winRate}% de efectividad). Se recomienda mantener la presión en el fondo de pista.`;
  }
  ...
  if (intent === "CMD_PARTNER_SYNC") {
      const partner = Analyzer.findPartner(u.id);
      if(!partner) return currentPersonality === 'vecina' 
        ? "Cariño, estás más solo que la una. ¡Búscate una pareja fija o un compañero de fatigas!" 
        : "No se ha detectado un compañero frecuente con alta sinergia estadística.";
      ...
      const pLvl = (partner.nivel || 2.5).toFixed(2);

      if (currentPersonality === 'vecina') return `¡Hacéis una pareja explosiva con **${pName}** (Nivel ${pLvl})! Habéis ganado el ${rate}% de vuestros ${total} partidos juntos. ¡Eso es química pura, corazón!`;
      return `Sinergia de Equipo: Tu socio estratégico es **${pName}** (Nivel ${pLvl}). WinRate conjunto: ${rate}% en ${total} enfrentamientos. Sincronización táctica: ÓPTIMA.`;
  }
  ...
  if (intent === "CMD_TUTORIAL") {
      return currentPersonality === 'vecina'
        ? "Es muy fácil, mi vida. En 'Calendario' te apuntas a partidos. En 'Ranking' cotilleas quién va primero (¡y quién va último!). En 'Diario' apuntas tus reflexiones de crack. ¡Si necesitas más, grítame!"
