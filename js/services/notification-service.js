...
export async function initAutoNotifications(uid) {
  ...
  logInfo("notifications_auto_init", { uid });
        ...
        if (diff !== 0) {
          ...
          if (posNow && posPrev && posNow < posPrev) {
            ...
            await createNotification(
              uid,
              "Ranking actualizado",
              `Has subido ${gain} ${gain === 1 ? "posicion" : "posiciones"} en el ranking.`,
          ...
          if (levelNow > levelPrev + 0.001) {
            ...
            await createNotification(
              uid,
              "Ajuste de nivel",
              `Tu nivel bajo: ${levelPrev.toFixed(2)} -> ${levelNow.toFixed(2)}. Revisa tu diario para recuperar ritmo.`,
        ...
        if (change.type === "modified") {
          ...
          if (realCount === 4) {
            ...
            await createNotification(
              uid,
              "Partido completo",
              `La partida del ${fechaStr} a las ${horaStr} ya esta completa (4/4).`,
  ...
  for (const colName of collections) {
    try {
      ...
      snapshot.forEach((d) => {
        ...
        const body = isOwner
          ? "Tu partido ya deberia haber terminado. Anade el resultado para actualizar ranking."
          : "Tu partido termino y aun no tiene resultado. Recuerda al organizador que lo registre.";
      ...
      if (change.type === "added") {
        ...
        if (createdAt >= fiveMinAgo) {
          await createNotification(
            uid,
            "Nuevo reto",
            "Te han incluido en un reto oficial. Aceptas el desafio?",
...
async function checkMorningMatchSummary(uid) {
  ...
  try {
    ...
    if (matchesToday > 0) {
      ...
      const name = user?.nombreUsuario?.split(" ")[0] || "Campeon";

      await createNotification(
        uid,
        `Hoy juegas, ${name}`,
        `Tienes ${matchesToday} partido(s) programado(s) para hoy. Mucha suerte.`,
        "info",
        null,
...
async function checkDailySummary(uid) {
  ...
  try {
    ...
    await createNotification(
      uid,
      `Buenos dias, ${userData.nombreUsuario?.split(" ")[0] || "Campeon"}`,
      message,
      "info",
      null,
      { type: "daily_summary" },
...
export function cleanupAutoNotifications() {
  ...
  logInfo("notifications_auto_cleanup");
}
...
export async function suggestDiaryEntry(uid, matchId, won) {
  ...
  try {
    ...
    if (won) {
      if (setWins.l === 0) note = "Victoria solida. Manten la presion y cierra en red.";
      ...
      else note = "Hoy costo cerrar. Prioriza saque y primera volea en el proximo.";
    }
  } catch (_) {
    note = won
      ? "Victoria trabajada. Manten la rutina y la concentracion."
      : "Buen partido. Analiza que mejorar y vuelve a intentarlo.";
  }
  ...
  const message = `${base}${score} ${note} Quieres registrarlo en tu diario tactico?`;
