...
function runLocalAIMotor(prompt) {
  ...
  if (phase === "chat") {
    ...
    if (q.includes("resumen") || q.includes("summary") || q.includes("mi ranking") || q.includes("estado")) {
      ...
      // Extract tech stats from diary entries
      const diaryTexts = (prompt.diary || []).map(d => String(d.text || "").toLowerCase());
      const hasFatigue = diaryTexts.some(t => t.includes("cansancio") || t.includes("fatiga"));
      const hasBandeja = diaryTexts.some(t => t.includes("bandeja"));
      const hasVibora = diaryTexts.some(t => t.includes("vibora"));

      let techStats = "";
      if (hasFatigue || hasBandeja || hasVibora) {
        techStats = `\n📉 Notas de campo:\n${hasBandeja ? '🎾 Bandeja: en progresión\n' : ''}${hasVibora ? '⚡ Víbora: técnica detectada\n' : ''}${hasFatigue ? '🔋 Cansancio: monitorizado\n' : ''}`;
      }

      return `🎮 ${user.name} — Resumen Competitivo\n\n🏆 ELO: ${user.points} puntos\n⭐ Nivel: ${user.level}\n🔥 Racha actual: ${user.streak > 0 ? '+' : ''}${user.streak}\n📊 Partidos totales: ${stats.totalMatches || 0} (${stats.playedMatches || 0} jugados)\n🎯 Winrate reciente: ${winrate}% (${stats.recentWins || 0}W / ${stats.recentLosses || 0}L)${techStats}\n📈 Forma reciente: ${formStr || 'Sin datos'}\n\n${user.streak >= 3 ? '💪 ¡Gran racha! Mantén la intensidad.' : user.streak <= -3 ? '🔄 Racha negativa. Enfoca en consistencia y evita errores no forzados.' : '⚖️ Forma estable. Buen momento para buscar rivales de nivel similar.'}`;
    }
